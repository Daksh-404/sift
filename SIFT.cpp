...
meSIFT::meSIFT(Mat src,int octaves,int intervals)
{
	srcImage = src.clone();
...
void meSIFT::BuildScaleSpace()
{
	...
	for (i = 0; i < m_numOctaves; i++)
	{
		...
		for (j = 1; j<int(m_numIntervals+3); j++)
		{
			...
			//imshow(to_string(10*j+i),m_glist[i][j]);
			//waitKey(0);
			//imshow(to_string(10*j+i),m_dogList[i][j-1]);
			//waitKey(0);
		}
		if (i < m_numOctaves - 1)
		{
...
void meSIFT::DetectExtrema()
{
	...
	for (int i = 0; i < m_numOctaves; i++)
	{
		...
		for (int j = 1; j < m_numIntervals + 1; j++)
		{
			...
			//imshow(to_string(10*j+i),m_extrema[i][j-1]);
			//waitKey(0);
		}
	}
...
// cutoff value.
int meSIFT::GetKernelSize(double sigma, double cut_off)
{
	int i;
...
void meSIFT::AssignOrientations()
{
	int i, j, k, xi, yi;
	int kk, tt;
	...
	for (int i = 0; i < m_numOctaves; i++)
	{
		...
		for (int j = 1; j < m_numIntervals + 1; j++)
		{
			...
			int h = meSIFT::GetKernelSize(1.5 * abs_sigma) / 2;
...
// addition earlier.
Mat meSIFT::BuildInterpolatedGaussianTable(int size, double sigma)
{
	int i, j;
	...
	for (i = 0; i < size; i++)
	{
		for (j = 0; j < size; j++)
		{
			temp = meSIFT::gaussian2D(i - half_kernel_size, j - half_kernel_size, sigma);
...
double meSIFT::gaussian2D(double x, double y, double sigma)
{
